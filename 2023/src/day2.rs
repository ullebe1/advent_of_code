...
pub fn input_generator(input: &str) -> Input {
    ...
    let games = lines.map(|line| { 
        ...
        let rounds = all_rounds.split("; ").map(|round| {
            println!("{}", round);
            let (mut red, mut green, mut blue) = (None, None, None);
            let colors = round.split(", ").map(|color| {
                println!("{}", color);
                let (number, color) = scan_fmt!(color, "{d} {}", usize, String).unwrap();
                match color.as_str() {
...
pub fn part1(input: &Input) -> usize {
    ...
    for game in input.games.clone() {
        ...
        for round in game.rounds {
            if round.red.unwrap_or(0) > 12 || round.green.unwrap_or(0) > 13 || round.blue.unwrap_or(0) > 14 {
                impossible = true;
            }
        }
...
pub fn part2(input: &Input) -> usize {
    ...
    for game in input.games.clone() {
        ...
        println!("{}: {} * {} * {} = {}", game.id, red.unwrap(), green.unwrap(), blue.unwrap(), power);
        sum += power;
    }
