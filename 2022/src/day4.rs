...
pub fn input_generator(input: &str) -> Input {
    let pairs = input
        ...
        .map(|line| {
            ...
            let (elf_1, elf_2) = (pairs[0].split('-').into_iter().map(|x| x.parse::<usize>().unwrap()).collect::<Vec<usize>>(), pairs[1].split('-').into_iter().map(|x| x.parse::<usize>().unwrap()).collect::<Vec<usize>>());
